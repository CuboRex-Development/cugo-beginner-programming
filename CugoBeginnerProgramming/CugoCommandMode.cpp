...
//グローバル変数宣言
//cmd_matrix関連
  long int count_cmd_matrix[CMD_SIZE][2];
  int flag_cmd_matrix[CMD_SIZE][4];
...
{
  //Serial.println(F("#   set_cmd_matrix"));//確認用
  count_cmd_matrix[init_current_cmd][0] = cmd_0;//L側目標カウント数
  count_cmd_matrix[init_current_cmd][1] = cmd_1;//R側目標カウント数
  flag_cmd_matrix[init_current_cmd][0] = cmd_2;//milisecがEXCEPTION_NO以外なら待ち
  flag_cmd_matrix[init_current_cmd][1] = cmd_3;//255ならボタンまち
  flag_cmd_matrix[init_current_cmd][2] = cmd_4;//L側上限速度
  flag_cmd_matrix[init_current_cmd][3] = cmd_5;//R側上限速度

}

void init_CMD()
{
  //Serial.println(F("#   init_CMD"));//確認用

  for (int i = 0; i < CMD_SIZE; i++)
  {
    init_current_cmd = i;
    set_cmd_matrix(EXCEPTION_NO, EXCEPTION_NO, EXCEPTION_NO, EXCEPTION_NO, EXCEPTION_NO, EXCEPTION_NO);
  }
...
void set_wait_time_cmd()
{
  ...
  //target_wait_time = micros() + flag_cmd_matrix[current_cmd][0] * 1000;
  target_wait_time = flag_cmd_matrix[current_cmd][0];
...
void wait_time(long int milisec)
{
  ...
  if (cmd_init == false)
  {
    ...
    set_cmd_matrix(EXCEPTION_NO, EXCEPTION_NO, milisec, EXCEPTION_NO, 0, 0);
    }else{
...
void check_achievement_wait_time_cmd( )
{
  ...
  if (target_wait_time < micros())
  {
    ...
    Serial.print(String(flag_cmd_matrix[current_cmd][0] + (micros() - target_wait_time) / 1000 ));
...
void wait_button()
{
  ...
  if (cmd_init == false)
  {
    ...
    // 初回起動時の処理
    set_cmd_matrix(EXCEPTION_NO, EXCEPTION_NO, EXCEPTION_NO, 255, 0, 0);
    if (COMMAND_MODE == run_mode) {
      Serial.print(F("###"));
      if (init_current_cmd < 9)
        Serial.print(F("0"));
...
void go_backward(float distance, float max_velocity)
{
  ...
  if (cmd_init == false)
  {
    ...
    set_cmd_matrix(-target_count_L, -target_count_R, EXCEPTION_NO, EXCEPTION_NO, -velocity, -velocity);
    if (COMMAND_MODE == run_mode) {
      Serial.print(F("###"));
      if (init_current_cmd < 9)
        Serial.print(F("0"));
...
void go_forward(float distance, float max_velocity)
{
  if (cmd_init == false)
  {
    ...
    set_cmd_matrix(target_count_L, target_count_R, EXCEPTION_NO, EXCEPTION_NO, velocity, velocity);
    //Serial.println("matrix_target_count_L/R: " + String(count_cmd_matrix[init_current_cmd][0]) + ", " + String(count_cmd_matrix[init_current_cmd][0]));
    if (COMMAND_MODE == run_mode) {
      Serial.print(F("###"));
      if (init_current_cmd < 9)
        Serial.print(F("0"));
...
{
  //Serial.println(F("#   reset_command_mode_flags"));//確認用
  cmd_init = false;
  current_cmd = 0;
  cmd_exec = false;
  count_done = false;
...
void set_go_forward_cmd( )
{
  ...
  target_count_L = current_count_L + count_cmd_matrix[current_cmd][0];//★

  //Serial.println("target_count_L: " + String(target_count_L) + " = " + String(motor_controllers[MOTOR_LEFT].getCount()) + " + " + String(count_cmd_matrix[current_cmd][0]));
  //Serial.println("target_count_L: " + String(target_count_L) + " = " + String( current_count_L) + " + " + String(count_cmd_matrix[current_cmd][0]));
  //while(1);
  if (count_cmd_matrix[current_cmd][0] >= 0) {
    cmd_L_back = false;
  } else {
    cmd_L_back = true;
  }
  start_count_R = current_count_R;
  target_count_R = current_count_R + count_cmd_matrix[current_cmd][1];//★
  if (count_cmd_matrix[current_cmd][1] >= 0) {
    cmd_R_back = false;
  } else {
    cmd_R_back = true;
  }
...
void cmd_manager_flags_init( )
{
  ...
  if (count_cmd_matrix[current_cmd][0] == EXCEPTION_NO && count_cmd_matrix[current_cmd][1] == EXCEPTION_NO)
    count_done = true;

  if (flag_cmd_matrix[current_cmd][0] == EXCEPTION_NO)
    wait_done = true;

  if (flag_cmd_matrix[current_cmd][1] != 255)
    button_done = true;

  if (flag_cmd_matrix[current_cmd][1] < 1 || 7 < flag_cmd_matrix[current_cmd][1])
    spi_done = true;
  ...
  if (count_done == true && wait_done == true && button_done == true && spi_done == true)
  {
    ...
    Serial.print(F("番目のコマンド：開始  "));
      ...
      float degree = 0 ;
      float distance = 0 ;
      if(flag_cmd_matrix[current_cmd][0] != EXCEPTION_NO)//実際の待ち時間確認
      {
      Serial.print(String(flag_cmd_matrix[current_cmd][0]));
      Serial.print(F("ミリ秒　待つ"));
      }else if(flag_cmd_matrix[current_cmd][1] == 255)//ボタン押し待ち
      {
      Serial.print(F("ボタン　押し待ち"));
      }else if(count_cmd_matrix[current_cmd][0] >= 0 && count_cmd_matrix[current_cmd][1] >= 0)//前進
      {
      distance =  (count_cmd_matrix[current_cmd][0]) * (( 2 * wheel_radius_l * PI) / encoder_resolution );
      Serial.print(String(abs(distance)));
      Serial.print(F(" m  前進"));
      }else if(count_cmd_matrix[current_cmd][0]  <= 0  && count_cmd_matrix[current_cmd][1] >= 0)//左回り
      {
      degree = (2 * wheel_radius_l * PI * count_cmd_matrix[current_cmd][0] * 360) / (encoder_resolution * tread * PI);
      Serial.print(String(abs(degree)));
      Serial.print(F(" 度　左回り"));
      }else if(count_cmd_matrix[current_cmd][0] >= 0 && count_cmd_matrix[current_cmd][1] <= 0)//右回り
      {
      degree = (2 * wheel_radius_l * PI * count_cmd_matrix[current_cmd][0] * 360) / (encoder_resolution * tread * PI);
      Serial.print(String(abs(degree)));
      Serial.print(F(" 度　右回り"));
      }else if(count_cmd_matrix[current_cmd][0]  <= 0  && count_cmd_matrix[current_cmd][1] <= 0)//後進
      {
      distance =  (count_cmd_matrix[current_cmd][0]) * (( 2 * wheel_radius_l * PI) / encoder_resolution );
      Serial.print(String(abs(distance)));
      Serial.print(F(" m  後進"));
...
void check_achievement_go_forward_cmd( )// motor_controllers[0] MOTOR_LEFT motor_controllers[1] MOTOR_RIGHT
{
  ...
  if (L_done == true && R_done == true){
    ...
    double degree = 0 ;
    double distance = 0 ;
    if (count_cmd_matrix[current_cmd][0] >= 0 && count_cmd_matrix[current_cmd][1] >= 0) //前進
    {
...
void cmd_manager( )
{
  ...
  else
  {
    ...
    if (cmd_exec == true) // elseにしないのは、スタートしてから実行したいため
    {
      ...
      if (flag_cmd_matrix[current_cmd][2] == 0 && flag_cmd_matrix[current_cmd][3] == 0)
      {
      unsigned char frame[10] = { 0xFF, 0x02, 0, 0, 0, 0, 0, 0, 0, 0 };
      ld2_float_to_frame(0, 2, frame);
      ld2_float_to_frame(0, 6, frame);
      ld2_write_cmd(frame);        
        ...
        // 各制御値の計算
        //l_count_p = count_cmd_matrix[current_cmd][0] - current_count_L;
        l_count_p = target_count_L - current_count_L;
        l_count_i = l_count_prev_i_ + l_count_p;
        l_count_d = l_count_p - l_count_prev_p_;
        //r_count_p = count_cmd_matrix[current_cmd][1] - current_count_R;
        ...
        l_count_gain = min( max(l_count_gain, -fabsf(flag_cmd_matrix[current_cmd][2])), fabsf(flag_cmd_matrix[current_cmd][2])); //ユーザ設定の速度上限
        r_count_gain = min( max(r_count_gain, -fabsf(flag_cmd_matrix[current_cmd][3])), fabsf(flag_cmd_matrix[current_cmd][3])); //ユーザ設定の速度上限
      ...
      if (count_done == true && wait_done == true && button_done == true)
      {
        ...
        if (end_command_mode == true)
        {
          Serial.println(F("###   コマンド実行終了          ###"));
          Serial.println(F("##################################"));
          end_command_mode = false;
          run_mode = RC_MODE;
          reset_command_mode_flags();          
    ...
    if (frame[1] == 0x80){  //5.5.1 Control Mode Feedback
      if(frame[2] == 0x00){
        ...
        if(old_run_mode == RC_MODE){
          ...
          old_run_mode = COMMAND_MODE;
          Serial.println(F("###   MODE:CUGO_COMMAND_MODE   ###"));          
...
void spi_cmd(int spi_cmd_value){
  ...
  if (cmd_init == false)
  {
    ...
    // 初回起動時の処理
    set_cmd_matrix( EXCEPTION_NO, EXCEPTION_NO, EXCEPTION_NO, spi_cmd_value, 0, 0);
    init_current_cmd++;
...
void check_achievement_spi_cmd(){
  ...
  //send_spi(flag_cmd_matrix[current_cmd][1]);
  spi_done = true;
  Serial.print(F("###"));
  if (current_cmd < 9)
    Serial.print(F("0"));
...
void view_cmd_matrix(){
  //Serial.println(F("#   view_cmd_matrix"));//確認用
  for (int i = 0; i < CMD_SIZE; i++)
  {
    Serial.println(count_cmd_matrix[i][0]);
    Serial.println(count_cmd_matrix[i][1]);
    Serial.println(flag_cmd_matrix[i][0]);
    Serial.println(flag_cmd_matrix[i][1]);
    Serial.println(flag_cmd_matrix[i][2]);
    Serial.println(flag_cmd_matrix[i][3]);
    Serial.println(i);
  }
...
void display_failsafe(bool FAIL_SAFE_DISPLAY){
  ...
  if (FAIL_SAFE_DISPLAY == true)
  {
    ...
    Serial.print(F("Mode(COMMAND/RC): "));
    Serial.println(run_mode);
  }
  }
...
void display_nothing(){//1000msごとに表示したいものがあれば記載
  ...
  if (ENCODER_DISPLAY == false && PID_CONTROLL_DISPLAY == false)
  {
    ...
    //Serial.println(F("Command is working..."));
    //Serial.println(F(""));
  }
  }
